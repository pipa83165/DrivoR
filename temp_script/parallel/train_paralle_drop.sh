...
NORMAL_EXPERIMENT="${NORMAL_EXPERIMENT:-paralle_normal_10ep}"
EVAL_SPLIT="${EVAL_SPLIT:-navtest}"
EXPERIMENT="${EXPERIMENT:-paralle_drop_${EVAL_SPLIT}}"
CKPT_PATH="${CKPT_PATH:-${NORMAL_CKPT:-}}"
AGENT=drivoR

if [ -z "$CKPT_PATH" ]; then
    CKPT_PATH=$(ls -t "${NAVSIM_EXP_ROOT}/ke/${NORMAL_EXPERIMENT}"/*/lightning_logs/version_*/checkpoints/last.ckpt 2>/dev/null | head -n 1 || true)
fi

if [ -z "$CKPT_PATH" ]; then
    echo "Error: normal checkpoint not found under ${NAVSIM_EXP_ROOT}/${NORMAL_EXPERIMENT}/"
    echo "Set CKPT_PATH=/path/to/normal/last.ckpt or NORMAL_EXPERIMENT=<experiment_name>."
    exit 1
fi
